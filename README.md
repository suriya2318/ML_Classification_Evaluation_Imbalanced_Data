...
[INSERT CHART 1 HERE — Class Distribution: Left panel = Bar chart showing Malignant (212) and Benign (357) patient counts with percentage labels. Right panel = Pie chart showing 37.3%/62.7% proportional split. Red = Malignant, Blue = Benign. Place this chart in the Dataset Overview section immediately after the class distribution table to visually demonstrate the imbalance before any modeling begins.]
...
Predicted MalignantPredicted BenignActual MalignantTrue Negative (TN)False Positive (FP) ← DANGEROUSActual BenignFalse Negative (FN)True Positive (TP)
...
[INSERT CHART 2 HERE — Confusion Matrix Heatmap for Logistic Regression with blue color scheme. 2×2 grid with Actual class on rows and Predicted class on columns. TN, FP, FN, TP labels annotated. Color intensity represents count magnitude. Place this chart in the Confusion Matrix section immediately after the confusion matrix breakdown table. This chart is mandatory and must appear before any Precision/Recall discussion begins.]
Precision, Recall & F1-Score Results
MetricLogistic RegressionBalanced LRDecision TreeAccuracy~97.37%~96.49%~93.86%Precision~97.18%~95.77%~92.86%Recall~98.59%~98.59%~95.77%F1-Score~97.88%~97.16%~94.30%ROC-AUC~0.9965~0.9972~0.9320
...
ModelROC-AUC ScoreClassificationLogistic Regression~0.9965Excellent discriminationBalanced LR~0.9972Excellent — marginally higherDecision Tree~0.9320Good but notably lowerRandom (baseline)0.5000No discrimination
[INSERT CHART 3 HERE — ROC Curves for all 3 models plotted on the same axes. Blue = Logistic Regression, Teal = Balanced LR, Amber = Decision Tree. Black dashed diagonal line = random classifier (AUC = 0.50). AUC score for each model shown in legend. Shaded area under Logistic Regression curve. Place this chart in the ROC Curve & AUC section immediately after the AUC results table. This is one of the most important charts in the entire report.]
Class Imbalance Handling Results
MetricBaseline LRBalanced LRChangeAccuracy~97.37%~96.49%-0.88%Precision~97.18%~95.77%-1.41%Recall~98.59%~98.59%0.00%F1-Score~97.88%~97.16%-0.72%ROC-AUC~0.9965~0.9972+0.0007
...
[INSERT CHART 5 HERE — Confusion Matrix Heatmap for Decision Tree Classifier with orange color scheme. Same 2×2 format as Chart 2 but visually distinct. Place this chart in the Decision Tree Comparison section after the Decision Tree classification report. Comparison with Chart 2 reveals Decision Tree makes more False Positive errors — confirming Logistic Regression is the safer medical deployment choice.]
Final Model Comparison
[INSERT CHART 4 HERE — Final Metrics Comparison grouped bar chart showing all 5 metrics (Accuracy, Precision, Recall, F1-Score, ROC-AUC) for all 3 models side by side. Blue = Logistic Regression, Teal = Balanced LR, Amber = Decision Tree. Y-axis starts at 0.80 to zoom into meaningful differences. Each bar labeled with exact score. Place this chart in the Final Model Comparison section after the complete 3-model comparison table to provide immediate visual confirmation of the winning model across all metrics simultaneously.]
